...
class Grafo:
    ...
    def calcPulo(self, caminhos, indice):
        if caminhos[indice][1] == 1:
            return 1
        else:
            return caminhos[ caminhos[indice][1]-1 ][2] + 1

    def dijkstra(self, origem):
        custo_vem = [[-1, 0, 0] for i in range(self.vertices)]
        custo_vem[origem - 1] = [0, origem, 0]
        h = HeapMax()
            ...
            for i in range(self.vertices):
                if self.grafo[v-1][i] != 0:
                    if custo_vem[i][0] == -1 or custo_vem[i][0] > dist + self.grafo[v-1][i]:
                        custo_vem[i] = [dist + self.grafo[v-1][i], v, 0]
                        h.adiciona_no(dist + self.grafo[v-1][i], i+1)

        for i in range(1, len(custo_vem)):
            custo_vem[i][2] = self.calcPulo(custo_vem, i)
...
for i in range(len(resultado_dijkstra)):
    print(i+1, ": [", resultado_dijkstra[i][0], resultado_dijkstra[i][2] ,"]")